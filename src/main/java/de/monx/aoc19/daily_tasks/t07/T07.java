...
public class T07 extends TDay {
	...
	void part1(String line) {
		...
		long max = executeAmtSeq(stack, permutations.getNext());
		int[] optSeq = seq.clone();
		while (permutations.hasNext()) {
			seq = permutations.getNext();
			long out = executeAmtSeq(stack, permutations.getNext());
			if (out > max) {
				max = out;
				optSeq = seq.clone();
			}
	...
	long executeAmtSeq(long[] stack, int[] seq) {
		IntCode amp = new IntCode();
		long out = 0;
		for (int i = 0; i < seq.length; i++) {
			amp.init(new int[] { seq[i], (int) out }, stack.clone());
...
//		int[] seq = { 9, 8, 7, 6, 5 };
		Permutations permutations = new Permutations(seq);
		seq = permutations.getFirst();
		long max = executeAmtSeqIO(stack, permutations.getNext());
		int[] optSeq = seq.clone();
		while (permutations.hasNext()) {
			seq = permutations.getNext();
			long out = executeAmtSeqIO(stack, permutations.getNext());
			if (out > max) {
				max = out;
				optSeq = seq.clone();
			}
		...
		// only tested with 2 tests inputs p2ex1 and p2ex2 from aoc day 7 part 2
		// examples
		System.out.println("Part2: OPtSeq:" + Arrays.toString(optSeq) + " -> " + max);
	}

	long executeAmtSeqIO(long[] stack, int[] seq) {
